...
export function AdminWidgetPreviewPage() {
    return (_jsxs("div", { className: "relative -m-4 h-[calc(100vh-6rem)] min-h-[480px]", children: [_jsx("span", { className: "absolute right-4 top-4 z-10 rounded-md border border-amber-300 bg-amber-100 px-2 py-1 text-xs font-medium text-amber-900 shadow", "aria-hidden": true, children: "Preview Mode" }), _jsx("iframe", { src: previewUrl, title: "Candidate Widget Preview", className: "h-full w-full rounded-md border border-slate-200 bg-white", sandbox: "allow-scripts allow-same-origin allow-forms allow-popups" })] }));
}
