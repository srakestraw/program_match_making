...
import { TraitPickerModal, type TraitForPicker } from "./TraitPickerModal";

const baseTraits: TraitForPicker[] = [
  {
    id: "trait-1",
...
function renderModal(overrides?: Partial<React.ComponentProps<typeof TraitPickerModal>>) {
  const onAddTraits = vi.fn(async () => {});
  const props: React.ComponentProps<typeof TraitPickerModal> = {
    isOpen: true,
    onClose: vi.fn(),
    traits: baseTraits,
    assignedTraitIds: new Set<string>(),
    programId: "program-1",
    degreeLevel: "Masters",
