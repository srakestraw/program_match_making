...
export function SelectedRail({
  ...
  return (
    <aside
      aria-label="Selected traits"
      className="flex h-full w-full flex-shrink-0 flex-col border-t border-slate-200/80 bg-slate-50/40 md:w-[300px] md:border-l md:border-t-0"
    >
        ...
        {selectedTraits.length === 0 ? (
          ...
          <ul className="space-y-1.5">
            {selectedTraits.map((trait) => (
              <li
                key={trait.id}
                className="flex items-center justify-between gap-2 rounded-md border border-slate-200/80 bg-white px-2 py-1.5 text-sm"
              >
                <span className="truncate text-slate-800">{trait.name}</span>
                <button
                  type="button"
