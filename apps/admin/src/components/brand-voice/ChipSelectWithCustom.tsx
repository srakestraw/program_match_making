...
export function ChipSelectWithCustom({
  ...
  return (
    <div>
      <label className="mb-2 block text-xs font-medium uppercase tracking-wide text-slate-600">{label}</label>
        ...
        {allOptions.map((item) => {
          ...
          return (
            <button
              ...
              onClick={() => onChange(selected ? value.filter((entry) => entry !== item) : [...value, item])}
            >
              {labelize(item)}
            </button>
          );
