...
const SelectorHarness = () => {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(LanguagePills, { valueTag: tag, customLanguage: custom, options: PRIMARY_LANGUAGE_OPTIONS, onChangeTag: (nextTag, nextLabel) => {
                    setTag(nextTag);
                    setLabel(nextLabel);
                }, onOpenOther: () => setOpen(true) }), _jsx(LanguagePickerModal, { open: open, options: EXTRA_LANGUAGE_OPTIONS, onClose: () => setOpen(false), onSelect: (nextTag, nextLabel) => {
                    setTag(nextTag);
                    setLabel(nextLabel);
                    setOpen(false);
                } })] }));
