...
export const TraitScorePanel = ({ traits, activeTraitId, done = false, onActiveTraitAction, actionPending = false }) => {
    ...
    return (_jsxs("section", { className: "rounded-xl border border-slate-200 bg-white/80 p-3", children: [_jsxs("div", { className: "mb-2 flex items-center justify-between", children: [_jsx("h3", { className: "text-sm font-semibold text-slate-900", children: "Trait scoring" }), _jsx("span", { className: "text-xs text-slate-500", children: "Live" })] }), _jsxs("div", { className: "max-h-72 space-y-2 overflow-y-auto pr-1", children: [ordered.map((trait) => {
                        ...
                        return (_jsxs("article", { className: `rounded-md border p-2 ${isActive ? "border-blue-400 bg-blue-50/50" : "border-slate-200 bg-white"}`, "data-testid": `trait-row-${trait.traitId}`, children: [_jsxs("div", { className: "mb-1 flex items-center justify-between gap-2", children: [_jsx("p", { className: "text-sm font-medium text-slate-900", children: trait.traitName }), _jsx("span", { className: `rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase ${statusClass[trait.status]}`, children: trait.status === "active" ? "in-progress" : trait.status })] }), isActive && onActiveTraitAction && !done && (_jsxs("div", { className: "mb-2 flex flex-wrap gap-2", children: [_jsx("button", { type: "button", className: "rounded-md border border-slate-300 px-2 py-1 text-[11px] font-semibold text-slate-700 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50", onClick: () => onActiveTraitAction("continue"), disabled: actionPending, children: "Continue" }), _jsx("button", { type: "button", className: "rounded-md border border-blue-300 bg-blue-100 px-2 py-1 text-[11px] font-semibold text-blue-700 hover:bg-blue-200 disabled:cursor-not-allowed disabled:opacity-50", onClick: () => onActiveTraitAction("deepen"), disabled: actionPending, children: "Go deeper" })] })), _jsxs("div", { className: "mb-1 flex items-center justify-between gap-2", children: [_jsx(SegmentedScore, { score: trait.score_1_to_5 }), trait.confidence && (_jsx("span", { className: `rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase ${confidenceClass[trait.confidence]}`, children: trait.confidence }))] }), evidence.length > 0 && (_jsx("ul", { className: "space-y-0.5 text-xs text-slate-700", children: evidence.map((entry, index) => (_jsx("li", { className: "truncate", children: entry }, `${trait.traitId}-evidence-${index}`))) })), _jsxs("div", { className: "mt-1 flex gap-3 text-xs", children: [trait.evidence.length > 2 && (_jsx("button", { type: "button", className: "text-slate-600 underline", onClick: () => setExpandedTraitIds((prev) => ({ ...prev, [trait.traitId]: !expanded })), children: expanded ? "Collapse evidence" : "Expand evidence" })), trait.rationale && (_jsx("button", { type: "button", className: "text-slate-700 underline", onClick: () => setExpandedTraitIds((prev) => ({ ...prev, [trait.traitId]: !expanded })), children: "Why this score?" }))] }), expanded && trait.rationale && _jsx("p", { className: "mt-1 text-xs text-slate-700", children: trait.rationale })] }, trait.traitId));
